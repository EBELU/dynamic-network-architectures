...
from dynamic_network_architectures.building_blocks.CBAM import CBAM
import numpy as np
...
class BasicBlockD(nn.Module):
    def __init__(self,
        ...
        # CBAM
        # ================   
        self.apply_cbam = cbam
        assert not (squeeze_excitation and cbam)
        
        if cbam: 
            self.cbam = CBAM(self.output_channels, conv_op)
    ...
    def forward(self, x):
        ...
        if self.apply_cbam: # <--- Tillagt
            out = self.cbam(out) # Utför
...
class StackedResidualBlocks(nn.Module):
    def __init__(self,
                 ...
                 is_bottleneck_stage: bool = False, # <--- Ändrat
                 stochastic_depth_p: float = 0.0,
                 squeeze_excitation: bool = False,
                 squeeze_excitation_reduction_ratio: float = 1. / 16,
                 cbam: bool = False # <--- Ändrat
                 ):
        """
        ...
        print("Initilized stracked res conv block")
        if block == BasicBlockD:
            blocks = nn.Sequential(
                block(conv_op, input_channels, output_channels[0], kernel_size, initial_stride, conv_bias,
                      ...
                      squeeze_excitation, squeeze_excitation_reduction_ratio, cbam),
                *[block(conv_op, output_channels[n - 1], output_channels[n], kernel_size, 1, conv_bias, norm_op,
                        ...
                        squeeze_excitation, squeeze_excitation_reduction_ratio, cbam) for n in range(1, n_blocks)]
            )
        ...
        self.blocks = blocks
        if is_bottleneck_stage:
            self.blocks.add_module("context_conv", nn.Conv3d(
                output_channels[-1],
                output_channels[-1],
                kernel_size=3,
                padding=2,
